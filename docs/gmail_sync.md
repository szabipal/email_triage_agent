...
Gmail sync is local-first and read-only. It imports recent unread messages,
processes them through the existing triage pipeline, and leaves calendar
execution disabled unless you separately configure it.
...
Imported messages are deduplicated by Gmail message ID.
